...
class ECG5000Dataset():
    def __init__(self, data_dir, split="TRAIN"):
        """
        Args:
            data_dir (str): Path to the directory containing the ECG5000 files.
        ...
        file_path = os.path.join(data_dir, f"ECG5000_{self.split}.arff")
        
        # Load and parse the ARFF file
        print(f"Loading ECG5000 {self.split} split...")
        ...
        # ECG5000 packs the sequence into columns, with the last column being the class label
        ...
        # ECG5000 is univariate, so num_features = 1
        ...
        # We decode them if necessary and convert to 0-indexed integers (0 to 4)
        if isinstance(raw_labels[0], bytes):
            raw_labels = numpy.array([lbl.decode('utf-8') for lbl in raw_labels])
            
        # Convert string/float labels to integers and map from (1 to 5) to (0 to 4) range
